...
$.richFilemanagerPlugin = function(element, pluginOptions)
{
	...
	var $container = $(element),	// reference to the jQuery version of DOM element the plugin is attached to
		...
		/** variables to keep request options host **/
	...
	fm.setDimensions = function() {
		var padding = $wrapper.outerHeight(true) - $wrapper.height(),
        	newH = $($container).height() - $header.height() - $footer.height() - padding,
            newW = $splitter.width() - $splitter.children('.splitter-bar-vertical').outerWidth() - ((config.filetree.enabled) ? $filetree.outerWidth() : 0) ;
	...
	var includeAssets = function(callback) {
        ...
        primary.push('../../plug-in/filemanager/themes/' + config.options.theme + '/styles/theme.css');

        if(config.viewer.image.lazyLoad) {
            primary.push('/lazyload/dist/lazyload.min.js');
        }
		...
		if(config.viewer.markdownRenderer.enabled) {
            secondary.push('/fm-markdown.css');
		...
		if(!config.options.browseOnly) {
            ...
            secondary.push('/libs-fileupload.js');
	...
	var initialize = function () {
		...
		fmModel = new FmModel();
        if (!ko.dataFor(document.getElementById('fileManager'))) {
            ko.applyBindings(fmModel, document.getElementById('fileManager'));
        }
		...
		doc.setAttribute('host-useragent', navigator.userAgent);
		...
		var PreviewModel = function() {
            ...
            this.applyObject = function(resourceObject) {
                ...
                if(isOpenDocFile(filename) && config.viewer.opendoc.enabled === true) {
                    ...
                    viewerObject.url = fm.settings.libsUrl + '/ViewerJS/index.html#' + createPreviewUrl(resourceObject, true);
                    viewerObject.options = {
            ...
            this.initiateLazyLoad = function() {
                ...
                items_model.lazyLoad = new LazyLoad({
                    ...
                    callback_load: function (element) {
                        fm.console('LOADED', element.getAttribute('host-original'));
                    },
                    callback_set: function (element) {
                        fm.console('SET', element.getAttribute('host-original'));
                    },
        ...
        var EditorModel = function() {
            ...
            function includeAssets(extension) {
                ...
                if (config.editor.codeHighlight) {
                    if (extension === 'js') {
                        assets.push('/CodeMirror/mode/javascript/javascript.js');
                        currentMode = 'javascript';
                    }
                    if (extension === 'css') {
                        assets.push('/CodeMirror/mode/css/css.js');
                        currentMode = 'css';
                    }
                    if (extension === 'html') {
                        assets.push('/CodeMirror/mode/xml/xml.js');
                    ...
                    if (extension === 'php') {
                        assets.push('/CodeMirror/mode/htmlmixed/htmlmixed.js');
                        assets.push('/CodeMirror/mode/xml/xml.js');
                        assets.push('/CodeMirror/mode/javascript/javascript.js');
                        assets.push('/CodeMirror/mode/css/css.js');
                        assets.push('/CodeMirror/mode/clike/clike.js');
                        assets.push('/CodeMirror/mode/php/php.js');
                    ...
                    if (extension === 'sql') {
                        assets.push('/CodeMirror/mode/sql/sql.js');
                        currentMode = 'text/x-mysql';
                    }
                    if (extension === 'md') {
                        assets.push('/CodeMirror/addon/mode/overlay.js');
                        assets.push('/CodeMirror/mode/xml/xml.js');
                        assets.push('/CodeMirror/mode/markdown/markdown.js');
                        assets.push('/CodeMirror/mode/gfm/gfm.js');
                        assets.push('/CodeMirror/mode/javascript/javascript.js');
                        assets.push('/CodeMirror/mode/css/css.js');
                        assets.push('/CodeMirror/mode/htmlmixed/htmlmixed.js');
                        assets.push('/CodeMirror/mode/clike/clike.js');
                        assets.push('/CodeMirror/mode/shell/shell.js');
                        assets.push('/CodeMirror/mode/meta.js');
                        currentMode = 'gfm';
                    }
                    if (extension === 'sh') {
                        assets.push('/CodeMirror/addon/mode/overlay.js');
	...
	var loadAssets = function(assets) {
        for (var i = 0, l = assets.length; i < l; i++) {
			if(typeof assets[i] === 'string') {
                assets[i] = fm.settings.libsUrl + assets[i];
			}
        }
    ...
    var formatTimestamp = function (datetime) {
        ...
        // Timezone support requires "iana-tz-host" package:
  ...
  var getFilteredFileExtensions = function() {
                ...
                if(response.data) {
                    ...
                    if(sourceNode) {
                        if(sourceNode.rdo.type === 'folder') {
                            ...
                            // update object host for the current and all child nodes
    ...
    var moveItemPrompt = function(objects, successCallback) {
        ...
        fm.prompt({
            ...
            template: {
                dialogInput:
                '<input host-alertify-input type="text" value="" />' +
	...
	var summarizeItems = function() {
		if (!fm.settings.summaryModel.enabled){
					...
					.on('fileuploadadd', function(e, data) {
						...
						$.each(data.files, function (index, file) {
							...
							var $template = $(tmpl('tmpl-upload-item', {
								file: file,
								lang: langModel.getTranslations(),
								imagesPath: fm.settings.libsUrl + '/jQuery-File-Upload/img'
							}));
