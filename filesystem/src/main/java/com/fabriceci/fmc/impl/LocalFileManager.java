...
//        ssif (!fileRoot.isEmpty() && fileRoot.endsWith("/")) {
...
//        }
            ...
            for (String f : files) {
                ...
                if (file.isDirectory()) {
                    if (isAllowedPattern(filename, file.isDirectory())) {
                        fileDataList.add(getFileInfo(filePath + File.separatorChar));
                    }
    ...
    FileData getFileInfo(String path) throws FileManagerException {
        ...
        if (file.isDirectory() && !path.endsWith(String.valueOf(File.separatorChar))) {
    ...
    public FileData actionAddFolder(String path, String name) throws FileManagerException {
        ...
        return getFileInfo(path + filename + File.separatorChar);
    }
    ...
    public FileData actionMove(String sourcePath, String targetPath) throws FileManagerException {
        ...
        File targetFile = getFile(targetPath + File.separatorChar + filename);

        String finalTargetPath = targetPath + filename + (sourceFile.isDirectory() ? File.separatorChar : "");
...
//            logger.error("Error serving image: " + file.getName(), e);
    ...
    public FileData actionRename(String sourcePath, String targetName) throws FileManagerException {
        ...
        int pathPos = sourcePath.lastIndexOf(File.separatorChar);
        ...
        if (targetName.contains(String.valueOf(File.separatorChar))) {
    ...
    public FileData actionSummarize() throws FileManagerException {
        ...
        try {
            attributes = getDirSummary(getFile(String.valueOf(File.separatorChar)).toPath());
        } catch (IOException e) {
            throw new FileManagerException(ClientErrorMessage.ERROR_SERVER);
        }

        FileData fileData = new FileData();
        fileData.setId(String.valueOf(File.separatorChar));
    ...
    public List<FileData> actionUpload(HttpServletRequest request, String path) throws FileManagerException {
        ...
        String targetDirectoryString = path.substring(0, path.lastIndexOf(File.separatorChar) + 1);
    ...
    private ArrayList<FileData>  uploadFiles(HttpServletRequest request, String targetDirectory) throws FileManagerException {
        ...
        try {
            for (Part uploadedFile : request.getParts()) {
                ...
                checkRestrictions(new File(targetDirectory + File.separatorChar + filename));
...
//                    logger.error("Wrong format for the property: 'upload.fileSizeLimit");
                ...
                String uploadedPath = getFile(targetDirectory).getAbsolutePath() + File.separatorChar + filename;
    ...
    public FileData actionCopy(String sourcePath, String targetDirPath) throws FileManagerException {
        ...
        String finalPath = targetDirPath + filename + (sourceFile.isDirectory() ? File.separatorChar : "");
    ...
    public List<FileData> actionExtract(String sourcePath, String targetPath) throws FileManagerException {
        ...
        try {
            ...
            while(zipEntry != null){
                ...
                File newFile = new File(targetDirFile.getAbsolutePath() + File.separatorChar + fileName);
                if(isMatchRestriction(newFile)){
                    ...
                    if(!FileUtils.getBaseName(fileName).startsWith(".")) {
                        int count = fileName.length() - fileName.replace(String.valueOf(File.separatorChar), "").length();
                        if(count < 1 || (count == 1 && fileName.endsWith(String.valueOf(File.separatorChar)))) {
            ...
            Files.walkFileTree(file.toPath(), new SimpleFileVisitor<Path>() {
                ...
                public FileVisitResult postVisitDirectory(Path dir, IOException exc) throws IOException {
                    if (exc == null) {
                        ...
                        if (isMatchRestriction(currentFile) && currentFile.getName().toLowerCase().startsWith(searchedTerm.toLowerCase())) {
                            fileDataList.add(getFileInfo(getRelativePath(currentFile) + File.separatorChar));
                        }
                        } catch (FileManagerException silent) {} finally {}
                        return FileVisitResult.CONTINUE;
                    } else {
